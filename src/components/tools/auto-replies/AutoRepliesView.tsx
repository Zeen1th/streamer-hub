...
export function AutoRepliesView() {
  ...
  const senderBadge = (() => {
    if (activeChatSender === 'bot') {
      return (
        <div
          className="flex items-center gap-1.5 rounded border border-emerald-500/30 bg-emerald-950/20 px-3 py-1.5 text-xs font-semibold text-emerald-400"
          title={t(lang, 'settings.senderBotActive', { name: activeChatSenderLogin || botLogin || 'Bot' })}
        >
    ...
    if (botAccountEnabled && preferredChatSender === 'bot' && !botConnected) {
      return (
        <div
          className="flex items-center gap-1.5 rounded border border-amber-500/30 bg-amber-950/20 px-3 py-1.5 text-xs font-semibold text-amber-300"
          title={t(lang, 'settings.senderBotOfflineFallback', { name: twitchChannel || 'Broadcaster' })}
        >
    ...
    return (
      <div
        className="flex items-center gap-1.5 rounded border border-primary/30 bg-primary/10 px-3 py-1.5 text-xs font-semibold text-ink"
        title={t(lang, 'settings.senderBroadcasterActive', { name: activeChatSenderLogin || twitchChannel || 'Broadcaster' })}
      >
