...
import { AlertCircle, Bot, Crown, Globe, MessageSquare, Pencil, Plus, Sparkles, Trash2, X } from 'lucide-react';
...
export function AutoRepliesView() {
  ...
  const testAi = async () => {
    ...
    if (result?.ok && result.message) {
      setTestState({ loading: false, text: result.message });
      useLogStore.getState().add({
        kind: 'chat',
        message: `[Simulated AI Reply] ${result.message}`,
      });
    } else {
      const err = result?.error ?? t(lang, 'autoReplies.aiFailed');
      setTestState({ loading: false, error: err });
      useLogStore.getState().add({
        kind: 'system',
        message: `[AI Test Failed] ${err}`,
      });
    }
  };
